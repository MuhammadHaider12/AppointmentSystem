...
const DoctorList = () => {
  ...
  const [appointedDoctorIds, setAppointedDoctorIds] = useState<Set<string>>(new Set())
  ...
  useEffect(() => {
    const loadDoctors = async () => {
      ...
      if (currentUserId) {
        ...
        const ids = new Set((existingData || []).map((row: any) => row.doctor_id).filter(Boolean))
        setAppointedDoctorIds(ids)
      } else {
        setAppointedDoctorIds(new Set())
      }
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {doctors.map((doctor) => (
            <div key={doctor.id} className="border rounded-lg p-4 hover:shadow-lg transition">
              ...
              {appointedDoctorIds.has(doctor.id) ? (
                <button
                  type="button"
                  disabled
                  className="mt-3 w-full text-center bg-green-100 text-green-800 py-2 rounded cursor-not-allowed"
                >
                  Already Appointed
                </button>
              ) : (
...
const BookAppointment = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id || !doctorId) {
      toast.error('Invalid booking request. Please try again.')
      setLoading(false)
      return
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:justify-between gap-3 py-3 lg:py-0 lg:min-h-16">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-sm text-gray-600 break-all">Welcome, {user?.email}</span>
