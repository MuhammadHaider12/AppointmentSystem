...
const DEFAULT_ADMIN_CREATED_PASSWORD = 'login123'
...
const DashboardStats = () => {
  ...
  const fetchStats = useCallback(async () => {
    const [
      ...
      supabase.from('appointments').select('patient_id').eq('status', 'pending')
    ])

    const pendingPatients = new Set(
      (pendingPatientsRes.data || []).map((row: any) => row.patient_id).filter(Boolean)
    ).size
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="min-w-[36rem] w-full divide-y divide-gray-200">
  ...
  const addDoctor = async (e: React.FormEvent) => {
    ...
    if (existingUser?.id) {
      ...
      const { error: signUpError } = await provisioningClient.auth.signUp({
        ...
        password: DEFAULT_ADMIN_CREATED_PASSWORD,
        options: {
          data: {
      ...
      if (signUpError && !/already registered/i.test(signUpError.message)) {
        toast.error('Failed to create doctor login account: ' + signUpError.message)
        setSaving(false)
        return
      }
    ...
    if (!userId) {
      toast.error('Could not create doctor profile row. Please check DB auth trigger and try again.')
      setSaving(false)
      return
    }
    ...
    if (updateError) {
      ...
      toast.success('Doctor account created/updated. Default password: login123')
      setShowAddForm(false)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={addDoctor} className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Doctor Login Email</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Doctor Login Password</label>
                <input
                  type="password"
                  placeholder="login123"
                  className="w-full border rounded p-2"
                  value={DEFAULT_ADMIN_CREATED_PASSWORD}
                  readOnly
                />
                <p className="text-xs text-gray-500 mt-1">Admin-created doctor accounts use temporary password: `login123`.</p>
              </div>
              <div>
...
const ManagePatients = () => {
  ...
  const fetchPatients = async () => {
    const { data, error } = await supabase
      .from('users')
      .select('id, full_name, email, phone, blood_group, allergies, is_active, created_at')
      .eq('role', 'patient')
      .order('created_at', { ascending: false })
  ...
  const addPatient = async (e: React.FormEvent) => {
    ...
    if (existingUser?.id) {
      ...
      if (signUpError && !/already registered/i.test(signUpError.message)) {
        toast.error('Failed to create patient login account: ' + signUpError.message)
        setSaving(false)
        return
      }
    ...
    if (!userId) {
      toast.error('Could not create patient profile row. Please check DB auth trigger and try again.')
      setSaving(false)
      return
    }
    ...
    if (updateError) {
      ...
      toast.success('Patient account created/updated. Default password: login123')
      setShowAddForm(false)
      setFormData({ full_name: '', email: '', phone: '', blood_group: '', allergies: '' })
      fetchPatients()
    }
  ...
  const togglePatientStatus = async (id: string, currentStatus: boolean) => {
    const { error } = await supabase
      .from('users')
      .update({ is_active: !currentStatus })
      .eq('id', id)
    ...
    if (error) {
      ...
      toast.success(`Patient ${!currentStatus ? 'activated' : 'deactivated'}`)
      fetchPatients()
    }
  }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={addPatient} className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Patient Login Email</label>
                <input
                  type="email"
              ...
              <p className="text-xs text-gray-500">Admin-created patient accounts use temporary password: `login123`.</p>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {patients.map((patient) => (
              <tr key={patient.id}>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-sm ${patient.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <td className="px-6 py-4">
                  <button
                    ...
                    className={`text-sm ${patient.is_active ? 'text-red-600' : 'text-green-600'}`}
                  >
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:justify-between gap-3 py-3 lg:py-0 lg:min-h-16">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
              ...
              <div className="flex flex-wrap gap-x-2 gap-y-1">
                ...
                <Link to="/admin/patients" className="text-gray-700 hover:text-gray-900 px-3 py-2">Manage Patients</Link>
                <Link to="/admin/appointments" className="text-gray-700 hover:text-gray-900 px-3 py-2">All Appointments</Link>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-sm text-gray-600 break-all">Admin: {user?.email}</span>
