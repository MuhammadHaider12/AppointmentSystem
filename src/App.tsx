...
function App() {
  ...
  const getMetadataRole = useCallback((user: any): string | null => {
  ...
  const fetchUserRole = useCallback(async (user: any) => {
    try {
      ...
      const timeout = new Promise<never>((_, reject) => {
        window.setTimeout(() => reject(new Error('role-fetch-timeout')), ROLE_FETCH_TIMEOUT_MS)
      })

      const { data, error } = await Promise.race([roleFetch, timeout]) as { data: any; error: any }

      if (!error && data) {
        setUserRole(data.role)
      } else {
        setUserRole(getMetadataRole(user))
      }
    } catch {
      setUserRole(getMetadataRole(user))
    } finally {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        try {
          ...
          // Token refresh should not re-run role lookup and flash loaders.
